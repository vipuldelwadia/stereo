...
public class DAAPEntry {
	...
	public static DAAPEntry parseStream(InputStream stream) throws IOException {
		...
		if (length > 4*1024*1024) {
			throw new IOException("read too large: " + length + " is " + length/1024/1024 + " MB");
		}
	...
	public DAAPEntry(int code, int length, byte[] bytes, int pos) throws IOException {
		...
		if (this.code == null) {
			...
			String name = "'" + new String(b) + "' " + String.format("%4x", code);
			name += ", length " + length + ", " + code;

			throw new IOException("parser: unknown entry " + name);
		}
		...
		case INTEGER:
			...
			break;
		case UINTEGER:
			this.value = readUInteger(bytes, pos);
			break;
		case SHORT:
		...
		case DATE:
			...
			cal.setTimeInMillis(readInteger(bytes, pos));
			this.value = cal;
			break;
		case LIST:
			int offset = 0;
	...
	private static long readUInteger(byte[] buffer, int pos) {
		long value = 0;
		for (int i = 0; i < 4; i++) {
			int shift = (4 - 1 - i) * 8;
			value += (buffer[pos+i] & 0x000000FF) << shift;
